...
class SeasonManager(@Autowired private val database: Database) {
    ...
    fun updateOverview(user: User, year: Int? = null, season: Int? = null) {
        ...
        val bound = if(year != null && season != null) {
            SeasonOverviewModal(year, season, year, season)
        }else{
            val data = database.from(Statistics).select(Statistics.key)
        ...
        when {
            ...
            compValue(bound.beginYear, bound.beginSeason) < compValue(overview.second.beginYear, overview.second.beginSeason) -> database.update(Statistics) {
                where { it.id eq overview.first }
                it.content to SeasonOverviewModal(bound.beginYear, bound.beginSeason, overview.second.endYear, overview.second.endSeason).toJSONString()
                it.updateTime to DateTimeUtil.now()
            }
            compValue(bound.endYear, bound.endSeason) > compValue(overview.second.endYear, overview.second.endSeason) -> database.update(Statistics) {
                where { it.id eq overview.first }
                it.content to SeasonOverviewModal(overview.second.beginYear, overview.second.beginSeason, bound.endYear, bound.endSeason).toJSONString()
                it.updateTime to DateTimeUtil.now()
            }
        }
    }
